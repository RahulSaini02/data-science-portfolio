...
const Certifications = ({data}) => {
  return (
    <section className='min-h-60 md:min-h-72 bg-gray-100 overflow-hidden w-full pb-8'>
      ...
      <div className='flex flex-1 items-center justify-around space-x-2 p-4 mx-auto md:max-w-7xl'>
        {
          data.map((certification) => (
              <Link href={`${certification.certificate}`} download={`${certification.certificate}`} target='_blank' key={certification.slug} className='hexagon w-32 md:w-48 h-32 md:h-48 bg-indigo-500 flex justify-center items-center shadow-lg text-center text-xs md:text-base p-2 font-bold text-gray-100 cursor-pointer'>{certification.title}</Link>
          ))
        }
      </div>
