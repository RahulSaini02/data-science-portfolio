  ...
  return (
    <section className="pb-8 md:pb-16 p-4 px-6 md:max-w-7xl mx-auto space-y-2 bg-white min-h-screen text-primary">
      ...
      <div className="space-y-4 my-4 lg:my-12">
        <h3 className="text-xl md:text-3xl lg:text-5xl font-heading font-bold tracking-wide text-left">
          {project.title}
        </h3>
        <p className="text-primary/80 text-sm md:text-lg lg:text-xl py-2 font-body  text-justify">
          {project.context}
        </p>
      </div>
      {/* Project Image */}
          ...
          {project?.links
            ? project?.links?.map((link) => (
                <Link
                  ...
                  href={link.url}
                  target="_blank"
                  className="group flex space-x-2 border border-primary/60 hover:border-secondary hover:text-white hover:bg-secondary hover:shadow-lg ease-in-out transition-all duration-125 rounded-md cursor-pointer p-2 items-center justify-center"
                >
                  {link?.label == "GitHub" ? (
                    <FaGithub className="text-primary/80 group-hover:text-white" />
                  ) : (
                    <FaLink className="text-primary/80 group-hover:text-white" />
                  )}
                  <p className="text-primary text-center group-hover:text-white">
                    {link.label}
                  </p>
                </Link>
              ))
        ...
        <div className="space-y-4 mt-8 md:w-1/3">
          {headings.map((content) => (
            <Link
              key={content.id}
              href={`#${content.id}`}
              className="flex items-center justify-between border-spacing-4 pb-3 border-b border-primary/60 cursor-pointer"
            >
